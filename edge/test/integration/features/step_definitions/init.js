...
var environment = config['test_03102'].env;
...
console.log('&test_03102 api: [' +  domains[environment] + ', ' + config['test_03102'].basepath + ']');

BeforeAll("BeforeFeatures", function(next) {
    if (apps[0].name) {
        console.log ("Getting Creds from DevAppsKeys.json");
        getCreds(config['test_03102'].app, config['test_03102'].product);
    } else
    {
...
Before(function(scenario, callback) {
    ...
    this.apickli = new apickli.Apickli(tokens[0],
        tokens[1]+ config['test_03102'].basepath,
        './test/integration/features/fixtures/');
    callback();
});
